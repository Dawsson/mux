...
`mux` is a configurable `zellij` session manager. In managed mode it generates layouts from `windows[]` config and runs each pane under a small supervisor so `logs`, `restart`, and `send` work even when the session was started detached.
...
- `src/pane-supervisor.ts` — default shell wrapper used in managed mode
